...
- Storage: `next-themes`' own key for Theme; `localStorage` (`<app-slug>-demo-preferences`, derived from `APP_SLUG`) for the demo fields. The app has no server-side preferences store
